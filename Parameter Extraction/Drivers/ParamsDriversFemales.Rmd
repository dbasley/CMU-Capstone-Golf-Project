---
title: "Extract Parameters for Driver Females"
output:
  html_document:
    df_print: paged
---

# Load Data and Filter to Only Driver Female Shots
...
df_driver_lim <- filter(df_st_club, Club == "Driver" & Sex == "Female")
```
...
for (height in heights) {
  for (side in sides) {
    ...
    df_ideal_driver[row, "Sex"] <- "Female"
    ...
    df_ideal_driver[row, "Data_Points"] <- nrow(shot_shape_df)
    ...
    df_ideal_driver[row, "CPA_LB"] <- median(shot_shape_df$`Club Path`) - (sd_cpa / 16)
    df_ideal_driver[row, "LA_LB"] <- median(shot_shape_df$`Swing Plane`) - (sd_la / 16)
    df_ideal_driver[row, "AA_LB"] <- mean(shot_shape_df$`Attack Angle`) - (sd_aa / 16)
    df_ideal_driver[row, "CS_LB"] <- mean(shot_shape_df$`Club Speed`) - (sd_cs / 16)
    df_ideal_driver[row, "SL_LB"] <- mean(shot_shape_df$shaft_lean) - (sd_sl / 16)
    df_ideal_driver[row, "FA_LB"] <- mean(shot_shape_df$`Face Angle`) - (sd_fa / 16)

    # Find upper bound for each parameter by adding a fraction of sd
    df_ideal_driver[row, "CPA_UB"] <- median(shot_shape_df$`Club Path`) + (sd_cpa / 16)
    df_ideal_driver[row, "LA_UB"] <- median(shot_shape_df$`Swing Plane`) + (sd_la / 16)
    df_ideal_driver[row, "AA_UB"] <- mean(shot_shape_df$`Attack Angle`) + (sd_aa / 16)
    df_ideal_driver[row, "CS_UB"] <- mean(shot_shape_df$`Club Speed`) + (sd_cs / 16)
    df_ideal_driver[row, "SL_UB"] <- mean(shot_shape_df$shaft_lean) + (sd_sl / 16)
    df_ideal_driver[row, "FA_UB"] <- mean(shot_shape_df$`Face Angle`) + (sd_fa / 16)
...
colnames(df_ideal_driver_mutated) <- c(
...
ggplot(df_driver_lim_height1, aes(x = `Max Height - Side`, y = `Max Height - Height`)) +
  ...
  labs(
    title = "Distribution of Shots at Max Height for a Driver - Females",
...
df_ideal_driver_female_sharable <- data.frame(
  Club = df_ideal_driver$Club,
  Shot_Shape = paste(df_ideal_driver$Height_Class, df_ideal_driver$Side_Class),
  Sex = df_ideal_driver$Sex,
  Club_Path_Angle = df_ideal_driver$Club_Path_Angle,
  CPA_LB = df_ideal_driver$CPA_LB,
  CPA_UB = df_ideal_driver$CPA_UB,
  Lie_Angle = df_ideal_driver$Lie_Angle,
  LA_LB = df_ideal_driver$LA_LB,
  LA_UB = df_ideal_driver$LA_UB,
  Attack_Angle = df_ideal_driver$Attack_Angle,
  AA_LB = df_ideal_driver$AA_LB,
  AA_UB = df_ideal_driver$AA_UB,
  Club_Speed = df_ideal_driver$Club_Speed,
  CS_LB = df_ideal_driver$CS_LB,
  CS_UB = df_ideal_driver$CS_UB,
  Shaft_Lean = df_ideal_driver$Shaft_Lean,
  SL_LB = df_ideal_driver$SL_LB,
  SL_UB = df_ideal_driver$SL_UB,
  Face_Angle = df_ideal_driver$Face_Angle,
  FA_LB = df_ideal_driver$FA_LB,
  FA_UB = df_ideal_driver$FA_UB,
  Data_Points = df_ideal_driver$Data_Points
)

# Output kable
kable(df_ideal_driver_female_sharable, escape = TRUE, caption = "Ideal Parameter Estimates for Driver From Centers of Actual Data (Males)") %>%
